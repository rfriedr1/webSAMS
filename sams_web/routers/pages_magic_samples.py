    ...
    if resolved is None:
        return RedirectResponse(
            url=append_magic_feedback(
                fallback,
                entered_value=raw,
                error_message="Invalid Magic Nav ID. Use 123, pr123, usr210, /prep, /graph, or /ana.",
            ),
            status_code=303,
        )
    ...
    if not exists:
        return RedirectResponse(
            url=append_magic_feedback(
                fallback,
                entered_value=raw,
                error_message=f"{resolved.kind.title()} #{int(resolved.identifier or 0)} was not found.",
            ),
            status_code=303,
        )
