...
- `sams_web/templates/*`: Jinja templates. `_detail_fields.html` holds the detail-page macros: headline cards, `render_detail_field_row`, and **`render_detail_sections`** — the secondary section grid every detail page renders through (never hand-write that loop again; it was copied five times before). `lab_queue.html` owns the lab queue tables and *includes* `_preparation_bench.html` / `_graphitization_bench.html`, matching the `preparation_bench.py` / `graphitization_bench.py` split.
...
- **Servers**: install from `requirements.lock.txt` (exact pins) + `pip install --no-deps -e .`, start with `start_server_windows.bat` (no `--reload`, no package installs, binds `0.0.0.0`, one worker, honours `.env`). The full procedure is `docs/server_installation.md` — **update it whenever the port, env vars, settings path, start command or dependencies change.** `start_webapp_*.{bat,sh}` are the **quick-start launchers** (`docs/quickstart.md`): they refuse to start without `.env`, install from `requirements.lock.txt` only when its hash changed (stamp in `.venv`), never override `SAMS_SETUP_DATA_FILE`, and run without `--reload` unless `SAMS_DEV=1`. Keep the three docs (`quickstart.md`, `server_installation.md`, README) in step with the launchers.
...
- **Design tokens**: type scale (`--text-xs` 0.72 · `--text-sm` 0.84 · `--text-base` 0.92 · `--text-md` 1 · `--text-lg` 1.18 · `--text-xl` 1.4 · `--text-2xl` 1.66 · `--text-3xl` 1.96 rem, plus `--text-h2/h3/h4`) and 4px spacing scale (`--space-1` … `--space-6`) live in `:root` in `style-core.css`. **Colours are semantic tokens too** — `--surface-tint`, `--brand-tint`, `--line-soft/-strong`, `--ink-strong/-soft`, `--brand-deep/-accent`, and the `--success-*` / `--danger-*` / `--warn-*` families, declared in `:root`. 380+ literals were folded onto them; near-identical shades (a few RGB units apart) were merged deliberately. A new hex literal is only acceptable when no token is within a hair of it. **Every text `font-size` must be a token** — the stylesheets were consolidated from 43 distinct literal sizes onto this scale, and a literal rem value is only acceptable for a glyph (stepper arrows, dirty dot, close ×, dropzone icon). Bands when choosing: xs = micro labels/badges, sm = labels, notes, table cells, base = field values and body copy, md = headings inside cards / buttons.
...
- Module layout in `sams_web/sample_import/`:
	...
	- `commit.py` — the single transaction, as named steps: `_validate_payload` (rejects before any write) → `_resolve_submitter` → `_resolve_invoice` → `_resolve_project` (new, or append to existing) → `_create_samples` (uses `_build_sample_payload` per row) → one `session.commit()`. Each step returns a small frozen dataclass (`_Party`, `_ProjectRef`, `_ImportPlan`); `commit_import` itself is just the sequence. `mailer.py` — render + SMTP send. `service.py` — glue for the router.
...
## Quick Validation
- Tests: `.venv/bin/python -m pytest -q` (needs the `dev` extras: `pip install -e '.[dev]'`). The suite was dead for months after the router split and the Submitter rename; keep it green — a suite that cannot import is worse than none.
